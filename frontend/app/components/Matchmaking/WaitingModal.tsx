...
const WaitingModal = ( { averageWaitingTime, modalStatus, handleClose, handleRetry, handleCancelSearch } :  any) => {
    ...
    <dialog id="my_modal_1" className="modal">
        ...
        <div className="modal-box text-center flex flex-col items-center justify-center">
            { modalStatus != "success" &&
            <div className="radial-progress m-4 text-success" style={{"--value":searchTimer * 100 / waitingTimeLimit}}>{searchTimer.toFixed(0)}</div>
            }
            { modalStatus == "success" &&
            <div className="radial-progress m-4 text-success" style={{"--value":redirectTimer * 100 / redirectTimeLimit}}>{redirectTimer.toFixed(0)}</div>
            }
            ...
            <p className="py-4 text-xs text-center">
                Average waiting time: {averageWaitingTime == null ? 0 : averageWaitingTime > 10 ? "10+" : averageWaitingTime.toFixed(1)}s
            </p>
            {modalStatus == "canceled" && <div style={{ display: 'flex', justifyContent: 'center' }}>
            <button
                className={`btn btn-outline btn-info btn-block mb-2 btn-active m-2`}
                onClick={() => handleRetry()}
            >
            Retry
            </button>
            <button
                className={`btn btn-outline btn-error btn-block mb-2 btn-active m-2`}
                onClick={() => handleClose()}
            >
            Close
            </button>
            ...
            {modalStatus == "searching" &&
            <button
                className={`btn btn-outline btn-error btn-block mb-2 btn-active`}
